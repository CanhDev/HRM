...
private apiUrl = `${environment.apiBaseUrl}ContractType`;
  getData(searchRequest: SearchRequest<ContractType>) : Observable<ApiResponseBasic>{
  ...
  create(data : ContractType) : Observable<ApiResponseBasic>{
    return this.http.post<ApiResponseBasic>(`${this.apiUrl}`, data);
  }
  update(id : number, data : ContractType) : Observable<ApiResponseBasic>{
