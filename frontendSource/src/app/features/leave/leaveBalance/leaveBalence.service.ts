...
export class LeaveBalenceService {

constructor(private http: HttpClient) { }
private apiUrl = `${environment.apiBaseUrl}EmployeeLeaveBalance`;
  getData(searchRequest: SearchRequest<EmployeeLeaveBalance>) : Observable<ApiResponseBasic>{
  ...
  create(data : EmployeeLeaveBalance) : Observable<ApiResponseBasic>{
    return this.http.post<ApiResponseBasic>(`${this.apiUrl}`, data);
  }
  update(id : number, data : EmployeeLeaveBalance) : Observable<ApiResponseBasic>{
